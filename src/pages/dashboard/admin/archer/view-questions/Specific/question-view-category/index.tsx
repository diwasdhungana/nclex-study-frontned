...
const FillBlankswithModes = ({ data, mode }: { data: any; mode: any }) => {
  ...
  useEffect(() => {
    if (contentRef.current) {
      const blanks = contentRef.current.querySelectorAll('.blank');
      blanks.forEach((blank, index) => {
        const answer = Array.isArray(data.correct) ? data.correct[index] : data.correct.toString();

        // Replace blank content with correct answer
        const answerSpan = document.createElement('span');
        answerSpan.textContent = answer;
        answerSpan.style.backgroundColor = '#e6ffe6';
        answerSpan.style.padding = '2px 5px';
        answerSpan.style.borderRadius = '4px';
        answerSpan.style.margin = '0 2px';
        answerSpan.style.display = 'inline-block';

        blank.replaceWith(answerSpan);
      });
    }
  }, []);
