...
import { Button, Group, Paper, ScrollArea, Stack, Text, Select } from '@mantine/core';
...
const QuestionDisplay = ({ props }: { props: any }) => {
  ...
  const { data: testdata, isLoading: testsLoading, isError: testError } = useGetMyTests();
    ...
    <Stack h="81vh" justify="space-between" gap="0px">
      ...
      <Group gap="xl" justify="space-between" mt="3px" mb="0px">
        <Group>
          ...
          <Button
            disabled={question.data.testStatus == 'completed'}
            onClick={() => {
              openModal();
            }}
          >
        ...
        <Group>
          <Button
            ...
            onClick={() => {
              navigate(
                `/dashboard/student/test?testId=${testId}&i=${prevIndex}&t=${totalQuestions}`
              );
            }}
          >
            Previous
          ...
          <Button
            ...
            onClick={() => {
              navigate(
                `/dashboard/student/test?testId=${testId}&i=${nextIndex}&t=${totalQuestions}`
              );
            }}
          >
            Next
